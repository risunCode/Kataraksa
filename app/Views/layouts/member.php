...
<head>
    ...
    <title><?= $title ?? 'Member - Kataraksa' ?></title>
...
<body class="bg-stone-50 min-h-screen">
    <div class="flex">
        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform -translate-x-full lg:translate-x-0 transition-transform duration-300">
            ...
            <div class="h-16 flex items-center justify-center border-b border-stone-200">
                <a href="<?= base_url('/member/dashboard') ?>" class="flex items-center gap-2">
            ...
            <nav class="p-4 space-y-1">
                ...
                $menuItems = [
                    ['url' => '/member/dashboard', 'icon' => 'layout-dashboard', 'label' => 'Dashboard'],
                    ['url' => '/member/borrowings', 'icon' => 'book-copy', 'label' => 'Peminjaman Saya'],
                    ['url' => '/member/catalog', 'icon' => 'library', 'label' => 'Katalog Buku'],
                ];
                ?>
                ...
                <!-- Link ke Katalog Publik -->
                <a href="<?= base_url('/catalog') ?>" target="_blank" class="flex items-center gap-3 px-4 py-3 text-slate-600 hover:bg-stone-100 rounded-lg transition-all">
                    ...
                    <div class="relative" id="user-dropdown-container">
                        <button id="user-dropdown-btn" class="flex items-center gap-3 hover:bg-stone-100 rounded-lg px-3 py-2 transition-all">
                            <div class="text-right hidden sm:block">
                                <p class="text-sm font-medium text-slate-800"><?= session()->get('user_name') ?? 'Member' ?></p>
                                <p class="text-xs text-emerald-600 font-medium">Member</p>
                            </div>
    ...
    <script>
        ...
        function confirmBorrow(url, bookTitle) {
            Swal.fire({
                title: 'Pinjam Buku?',
        ...
        Swal.fire({
            ...
            html: '<p class="text-lg">Halo, <strong><?= esc(session()->getFlashdata('swal_welcome')) ?></strong></p><p class="text-gray-500">Selamat membaca!</p>',
            timer: 3000,
            showConfirmButton: false,
